...
- [x] `ci.yml`: Rename binary output from `haproxy-monitor` to `gearbox` (line 111, 117-118)
- [x] `ci.yml`: Rename artifact name from `haproxy-monitor-${{ github.sha }}` to `gearbox-${{ github.sha }}`
- [x] `release.yml` `build-monitor-binaries` job: Rename to `build-dashboard-binaries`
- [x] `release.yml`: Rename binary from `haproxy-monitor-*` to `gearbox-*` in build step
- [x] `release.yml` `build-docker` job: Change image name from `haproxy-monitor` to `gearbox` (lines 232, 257-258)
- [x] `release.yml`: Verify `./cmd/server` is the correct entry point (may have been renamed)

### 1.2 Use `go-version-file` instead of hardcoded version

- [x] All workflows: Replace `go-version: '1.25'` with `go-version-file: gearbox/go.mod` or `gearbox-agent/go.mod`
- [x] Remove `cache-dependency-path` since `actions/setup-go@v6` infers cache from `go.mod` by default

This ensures Go version is managed in one place (`go.mod`) and Dependabot can bump it.

### 1.3 Pin tool versions

- [x] Pin `templ` to a specific version instead of `@latest` (use version from `go.mod`: `v0.3.977`)
- [x] Pin `golangci-lint-action` to a specific lint version instead of `version: latest`
- [x] Pin `gosec` action to a tagged release instead of `@master`
...
- [x] Add a step that verifies generated `*_templ.go` files are up-to-date (run `templ generate` then `git diff --exit-code` on generated files)
- [x] This catches cases where someone edited `.templ` files but forgot to regenerate

### 2.2 Add build verification for PRs

- [x] `ci.yml`: Make build job depend on lint + test (currently all three run independently)
- [x] Add `needs: [test, lint]` to the build job so it only runs if lint and test pass

### 2.3 Consolidate CI path filtering

- [x] `ci.yml`: Add path filters for `gearbox/**` (currently runs on all pushes to main/develop, even agent-only changes)
- [x] Both CI workflows: Add `.github/workflows/ci*.yml` to paths to re-trigger on workflow changes

### 2.4 Drop `develop` branch triggers (if not used)

- [x] Verify if `develop` branch is actively used; if not, remove from CI triggers to reduce noise
- [x] If used, keep but add path filters consistently
...
- [x] `security.yml`: Update all `github/codeql-action/upload-sarif@v3` to `@v4`
- [x] v3 is deprecated December 2026; v4 uses CodeQL bundle 2.24.0+

### 3.2 Add CodeQL analysis workflow

- [x] Create new workflow `codeql.yml` for deep static analysis
- [x] Configure for Go language
...
- [x] Add Trivy container image scan to `docker.yml` after build (scan the built image, not just the repo)
- [x] Add Trivy container image scan to `docker-agent.yml` after build
- [x] Upload SARIF results to GitHub Security tab

### 3.4 Pin GitHub Actions to SHA hashes

- [x] Pin all third-party actions to full commit SHAs (not mutable tags)
- [x] Add a comment with the version tag next to each SHA for readability
- [x] Example: `uses: actions/checkout@<sha> # v6`
- [x] Use StepSecurity or manual pinning for all workflows

### 3.5 Add `step-security/harden-runner` to workflows

- [x] Add `harden-runner` as the first step in security-sensitive jobs (release, Docker push)
- [x] Audit mode initially, block mode once baseline is established

### 3.6 Tighten permissions

- [x] Set top-level `permissions: {}` (deny all) in every workflow
- [x] Grant minimum required permissions per-job, not per-workflow
- [x] `security.yml`: Move `security-events: write` to only the jobs that need it
- [x] `release.yml`: Split permissions per job instead of broad workflow-level grants

### 3.7 Pin gosec to tagged version

- [x] Replace `securego/gosec@master` with a pinned SHA of the latest tagged release
- [x] `@master` is a security risk (mutable reference)
...
- [x] Install cosign in release workflow via `sigstore/cosign-installer@v3`
- [x] Sign release checksums file with cosign (keyless via GitHub OIDC)
- [x] Sign Docker images with cosign after push
- [x] Add verification instructions to release notes

### 4.2 Add SBOM generation to releases

- [x] Install Syft via `anchore/sbom-action/download-syft@v0`
- [x] Generate SBOM for each binary artifact (SPDX or CycloneDX format)
- [x] Attach SBOMs to GitHub release
- [x] Enable SBOM attestation on Docker images via `docker/build-push-action` `sbom: true`

### 4.3 Add build provenance attestation to binaries

- [x] Use `actions/attest-build-provenance@v3` for binary artifacts (not just Docker)
- [x] Achieves SLSA Build Level 2 for binaries

### 4.4 Add Docker image scanning to release pipeline

- [x] Scan release Docker images with Trivy before publishing
- [x] Fail the release if CRITICAL vulnerabilities are found in the image

### 4.5 Add agent Docker build to release pipeline

- [x] Verify `release.yml` `build-docker` job builds both gearbox and gearbox-agent images
- [x] Currently only builds dashboard image; add parallel job for agent image
...
- [x] New workflow: `monthly-release.yml`
- [x] Trigger: `schedule: cron '0 6 1 * *'` (1st of month, 6 AM UTC) + `workflow_dispatch`
- [x] Steps:
...
- [x] Script to determine next patch version from latest git tag
- [x] Format: if latest is `v1.2.3`, next monthly is `v1.2.4`
- [x] Manual releases can still use any version via tag push
- [x] Include `workflow_dispatch` input to override version if needed

### 5.3 Monthly release changelog

- [x] Auto-generate changelog from commits since last release
- [x] Include a note that this is an automated monthly release with updated system packages
- [x] List any dependency updates that were merged since last release
...
- [x] Delete `.github/renovate.json`
- [x] Dependabot will be the sole dependency manager (native GitHub, simpler)

### 6.2 Update Dependabot configuration

- [x] Add `gearbox-agent/` Go modules tracking (currently only `gearbox/`)
- [x] Add `gearbox-agent/` Docker base image tracking (currently only `gearbox/`)
- [x] Ensure all groups use `patterns: ["*"]` to bundle into single PRs
- [x] Add `npm` ecosystem for `gearbox/` (if `package.json` exists)
- [x] Set consistent schedule across all ecosystems
...
- [x] New workflow: `dependabot-auto-merge.yml`
- [x] Auto-merge patch and minor dependency updates after CI passes
- [x] Require manual review for major version bumps
- [x] Use `dependabot/fetch-metadata@v2` to check update type
- [x] Use `gh pr merge --auto --squash` for approved PRs
- [x] This means monthly you get ~3 grouped PRs max (Go deps, Actions, Docker) that auto-merge
...
- [x] New workflow: `scorecard.yml`
- [x] Uses `ossf/scorecard-action@v2`
- [x] Runs weekly on main branch
- [x] Publishes results to GitHub Security tab
- [x] Provides public supply chain security score

### 7.2 Add PR labeler workflow

- [x] New workflow or add to existing: auto-label PRs based on changed paths
- [x] Labels: `dashboard`, `agent`, `docs`, `ci`, `dependencies`
- [x] Helps with changelog generation and filtering
...
- [x] `release.yml` has a `build-docker` job that duplicates what `docker.yml` does on tag push
- [x] `docker.yml` already triggers on `v*` tags and builds with proper semver tags
- [x] **Option A**: Remove `build-docker` from `release.yml` entirely (let `docker.yml` + `docker-agent.yml` handle it)
- [ ] ~~**Option B**: Keep in `release.yml` but disable `docker.yml` tag trigger to avoid double builds~~
- [x] Recommended: **Option A** - cleaner separation of concerns
...
| `actions/dependency-review-action` | `@v6` | `@<sha>` (pin to v4 SHA) |
