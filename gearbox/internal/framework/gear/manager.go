...
type Manager struct {
	...
	enabled       map[string]map[string]bool // serverID -> gearName -> enabled
	store         GearStore
	eventHandlers map[string][]func(Event) // eventType -> handlers
}

// NewManager creates a new plugin manager.
func NewManager(deps Dependencies, store GearStore, logger *slog.Logger) *Manager {
	if logger == nil {
		logger = slog.Default()
	}
...
func (m *Manager) InitializeAll(ctx context.Context) error {
	plugins := All()
	m.logger.Info("initializing gears", "count", len(plugins))
	...
	for _, p := range plugins {
		...
		if m.store != nil {
			...
			if err != nil {
				m.logger.Warn("failed to load gear config", "gear", info.Name, "error", err)
			} else {
				gearDeps.Config = config
			}
		}
	...
	m.logger.Info("all gears initialized", "count", len(plugins))
	return nil
}
...
func (m *Manager) StartAll(ctx context.Context) error {
	plugins := All()
	m.logger.Info("starting gears", "count", len(plugins))
	...
	m.logger.Info("all gears started", "count", len(plugins))
	return nil
}
...
func (m *Manager) StopAll(ctx context.Context) error {
	...
	m.logger.Info("stopping gears", "count", len(plugins))
	...
	if len(errors) > 0 {
		return fmt.Errorf("failed to stop %d gears", len(errors))
	}

	m.logger.Info("all gears stopped")
	return nil
}
...
func (m *Manager) RegisterRoutes(r chi.Router) {
	...
	for _, p := range plugins {
		...
		if sidebar == nil {
			...
			m.logger.Debug("gear has no routes", "gear", info.Name)
			continue
		}

		m.logger.Debug("registering routes", "gear", info.Name, "path", sidebar.Path)
...
func (m *Manager) IsEnabled(gearName, serverID string) bool {
	// Check if it's a core gear (always enabled)
	if p, ok := Get(gearName); ok && p.Info().Core {
		return true
	}

	if m.store != nil {
		return m.store.IsEnabled(gearName, serverID)
	}
	...
	return serverEnabled[gearName]
}

// SetEnabled enables or disables a plugin for a server.
func (m *Manager) SetEnabled(gearName, serverID string, enabled bool) error {
	// Cannot disable core plugins
	if p, ok := Get(gearName); ok && p.Info().Core && !enabled {
		return fmt.Errorf("cannot disable core gear %q", gearName)
	}

	if m.store != nil {
		return m.store.SetEnabled(gearName, serverID, enabled)
	}
	...
	m.enabled[serverID][gearName] = enabled
	return nil
}
...
func (m *Manager) RunMigrations(ctx context.Context) error {
	plugins := All()
	m.logger.Info("running gear migrations", "count", len(plugins))
	...
	for _, p := range plugins {
		...
		m.logger.Debug("running migrations", "gear", info.Name, "count", len(migrations))
	...
	m.logger.Info("gear migrations complete")
	return nil
}

// isMigrationApplied checks if a migration has been applied.
func (m *Manager) isMigrationApplied(ctx context.Context, gearName string, version int) (bool, error) {
	if m.deps.DB == nil {
		return false, nil
	}
	...
	err := m.deps.DB.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM gear_migrations WHERE plugin_name = ? AND version = ?",
		gearName, version).Scan(&count)
...
func (m *Manager) applyMigration(ctx context.Context, gearName string, migration Migration) error {
	if m.deps.DB == nil {
	...
	_, err = tx.ExecContext(ctx,
		"INSERT INTO gear_migrations (plugin_name, version, description, applied_at) VALUES (?, ?, ?, ?)",
		gearName, migration.Version, migration.Description, time.Now())
	if err != nil {
...
// GetGearConfig returns the configuration for a specific gear.
func (m *Manager) GetGearConfig(gearName string) (map[string]any, error) {
	if m.store == nil {
		return nil, fmt.Errorf("no gear store configured")
	}
	return m.store.GetConfig(gearName)
}

// SaveGearConfig saves the configuration for a specific gear.
func (m *Manager) SaveGearConfig(gearName string, config map[string]any) error {
	if m.store == nil {
		return fmt.Errorf("no gear store configured")
	}
	return m.store.SaveConfig(gearName, config)
}
