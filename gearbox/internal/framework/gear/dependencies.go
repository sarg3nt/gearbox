...
type FullAuthChecker interface {
	...
	// GetGearUserPermissions retrieves the user's full permission set.
	// Returns a UserPermissions object with all component-action permissions.
	GetGearUserPermissions(r *http.Request) (*UserPermissions, error)
...
// GearStore provides storage operations for plugins.
// Gears can use this to persist their configuration and state.
type GearStore interface {
	// GetConfig retrieves the plugin's configuration.
	GetConfig(gearName string) (map[string]any, error)

	// SaveConfig saves the plugin's configuration.
	SaveConfig(gearName string, config map[string]any) error

	// IsEnabled checks if the plugin is enabled for the given server.
	IsEnabled(gearName, serverID string) bool

	// SetEnabled enables or disables the plugin for a server.
	SetEnabled(gearName, serverID string, enabled bool) error

	// GetOrder returns the display order for the gear.
	GetOrder(gearName, serverID string) int

	// SetOrder sets the display order for the gear.
	SetOrder(gearName, serverID string, order int) error
}
...
type ServerRegistry interface {
	...
	IsGearEnabled(serverID, integration string) bool
}
