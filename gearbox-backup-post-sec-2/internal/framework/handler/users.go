...
// ChangePasswordPage serves the change password form (for forced password change).
func (h *Handler) ChangePasswordPage(w http.ResponseWriter, r *http.Request) {
	...
	csrfToken, _ := h.authManager.GetCSRFToken(r)
	errorMessage := r.URL.Query().Get("error")
	forced := r.URL.Query().Get("forced") == "true"

	component := pages.ChangePassword(user, errorMessage, csrfToken, forced, auth.GetPasswordRequirements())
	if err := component.Render(r.Context(), w); err != nil {
...
func (h *Handler) ChangePasswordPost(w http.ResponseWriter, r *http.Request) {
	...
	forced := r.FormValue("forced") == "true"
	...
	if newPassword != confirmPassword {
		redirectURL := "/settings/change-password?error=Passwords+do+not+match"
		if forced {
			redirectURL += "&forced=true"
		}
	...
	var changeErr error
	if forced {
		// For forced password change, we also require email
		if newEmail == "" {
			http.Redirect(w, r, "/settings/change-password?error=Email+is+required&forced=true", http.StatusSeeOther)
			return
		}
		...
		if err != nil {
			...
			http.Redirect(w, r, "/settings/change-password?error=Failed+to+validate+email&forced=true", http.StatusSeeOther)
			return
		}
		if existingUser != nil && existingUser.ID != user.ID {
			http.Redirect(w, r, "/settings/change-password?error=Email+is+already+in+use&forced=true", http.StatusSeeOther)
			return
		}

		// For forced password change, we don't require current password but do require email
		changeErr = h.authManager.SetPasswordAndEmail(r, user.ID, newPassword, newEmail)
	} else {
		changeErr = h.authManager.ChangePassword(r, user.ID, currentPassword, newPassword)
	}

	if changeErr != nil {
		redirectURL := "/settings/change-password?error=" + url.QueryEscape(changeErr.Error())
		if forced {
			redirectURL += "&forced=true"
		}
		http.Redirect(w, r, redirectURL, http.StatusSeeOther)
