...
// Plugin implements the traffic analysis plugin.
type Plugin struct {
	plugin.BasePlugin
...
func (p *Plugin) HandleTraffic(w http.ResponseWriter, r *http.Request) {
	if p.statsClient == nil {
...
func (p *Plugin) collectTrafficFromStickTables(limit int) ([]TrafficSourceData, error) {
...
func (p *Plugin) collectBackendTraffic() ([]BackendTrafficData, error) {
...
func (p *Plugin) HandleTrafficSummary(w http.ResponseWriter, r *http.Request) {
	if p.statsClient == nil {
		http.Error(w, "HAProxy stats not configured", http.StatusServiceUnavailable)
		return
	}
